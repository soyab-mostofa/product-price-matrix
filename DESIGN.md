...
- **The `EDITED` marker is also the undo control, and it is tracked per field.** For an admin it is a button: violet at rest, crimson on hover and focus, where its label swaps to `Undo` with a drawn icon — the destructive reading is stated before the click, not after. Both faces occupy the same grid cell so the swap cannot change the chip's width and reflow the column under the pointer. Clicking restores a Local field's immutable workbook baseline; an Imported MRP resolves again from the current verified listings before falling back to its workbook reference. Source Cost and MRP each carry their own marker: editing one must never mark the other. Anonymous readers do not see admin edit-state marks.
- **Admin Excel export is icon-only and one file per book.** Separate Local and Imported actions each download one `.xlsx` with numeric price cells, frozen headers, filters, current engine output, immutable workbook baselines, provenance, and that book's marketplace channels. Both are hidden until login and require the same-origin admin header.
