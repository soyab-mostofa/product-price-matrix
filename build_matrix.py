...
function setDiscountType(type) {
  const oldType = globalCostParams.discountType || 'pct';
  const valInput = document.getElementById('inputDiscountVal');
  let currentVal = Number(valInput.value) || 0;
  ...
  if (oldType !== type) {
    ...
    const sampleBase = 1000 + overhead;
    const sampleList = sampleBase * (1 + Number(globalCostParams.targetMarginPct || 0) / 100);

    if (type === 'amt') {
      // Pct -> Amt: Convert percentage of list price to BDT amount
      if (sampleList > 0) {
        currentVal = Math.round((sampleList * currentVal) / 100);
      }
    } else {
      // Amt -> Pct: Convert BDT amount to percentage of list price
      if (sampleList > 0) {
        currentVal = Math.round((currentVal / sampleList) * 100);
      }
    }
    valInput.value = currentVal;
    globalCostParams.discountVal = currentVal;
  }
...
function setProdDiscountType(type, triggerSummary = true) {
  const oldType = currentProdDiscountType || 'pct';
  const valInput = document.getElementById('prodInputDiscountVal');
  let currentVal = Number(valInput.value) || 0;

  if (oldType !== type && activeProductDetail) {
    const mfg = Number(activeProductDetail.manufactured_price) || 0;
    ...
    const listPrice = (mfg + pkg + tr + del + cac) * (1 + margin / 100);

    if (type === 'amt') {
      // Pct -> Amt: Convert % to BDT amount based on product's list price
      if (listPrice > 0) {
        currentVal = Math.round((listPrice * currentVal) / 100);
      }
    } else {
      // Amt -> Pct: Convert BDT amount to % of product's list price
      if (listPrice > 0) {
        currentVal = Math.round((currentVal / listPrice) * 100);
      }
    }
    valInput.value = currentVal;
  }
